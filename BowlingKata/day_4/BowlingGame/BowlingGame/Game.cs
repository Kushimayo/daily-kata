namespace BowlingGame
{
    public class Game
    {
        ...
        public int score()
        {
            gameScore += NormalFrameScore(0);
            for (int i = 1; i < 10; i++)
            {
                for (int j = 0; j < rollPins[i].Length; j++)
                {
                    gameScore += rollPins[i][j];
                }

                if (i > 0 && i < TEN_FRAME)
                {
                    if (IsStrikeFrame(i - 1))
                    {
                        gameScore += NormalFrameScore(i);
                        if (i > 1 && IsStrikeFrame(i - 2))
                        {
                            gameScore += NormalFrameScore(i);
                        }
                    }
                    ...
                    if (IsStrikeFrame(i - 1))
                    {
                        if (rollPins[i][0] == 10)
                        {
                            ...
                            if (rollPins[i][1] == 10)
                                ...
                                gameScore = gameScore + rollPins[i][1] + rollPins[i][2];

                            
                            if (IsStrikeFrame(i - 2))
                                gameScore += 10;
                        }
                        else
                        {
